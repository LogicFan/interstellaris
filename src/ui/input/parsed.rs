...
/// A resource to store parsed input of the mouse.
#[derive(Debug, Default, Clone, Copy, Resource, Reflect)]
pub struct ParsedMouseInput {
...
impl ParsedMouseInput {
    /// Prepare for the new frame data.
    pub(super) fn refresh_motion(&mut self, viewport: Vec2) {
    ...
    pub(super) fn set_motion(&mut self, delta: Vec2) -> Vec2 {
        self.motion += delta;
    ...
    pub(super) fn update_motion(&mut self) {
