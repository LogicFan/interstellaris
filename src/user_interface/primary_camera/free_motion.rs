...
use super::PrimaryCameraMotionMode;
use super::PrimaryCameraOrigin;
use bevy::input::mouse::{MouseMotion, MouseWheel};
use bevy::prelude::*;
use bevy::window::PrimaryWindow;

/// Run condition for the system under this module.
pub fn is_free_motion(r_motion_mode: Res<PrimaryCameraMotionMode>) -> bool {
    match *r_motion_mode {
        PrimaryCameraMotionMode::FreeMotion { .. } => true,
        _ => false,
    }
}
...
pub fn move_main_camera(
    ...
    r_motion_mode: Res<PrimaryCameraMotionMode>,
    r_settings: Res<InputSettings>,
) {
    let max_r = match *r_motion_mode {
        PrimaryCameraMotionMode::FreeMotion { max_r, .. } => max_r,
...
pub fn zoom_main_camera(
    ...
    let (min_h, max_h) = match *r_motion_mode {
        PrimaryCameraMotionMode::FreeMotion { min_h, max_h, .. } => (min_h, max_h),
        other => panic!("Unexpected value {:?}", other),
    };
...
pub fn rotate_main_camera(
    ...
    let max_θ = match *r_motion_mode {
        PrimaryCameraMotionMode::FreeMotion { max_θ, .. } => max_θ,
        other => panic!("Unexpected value {:?}", other),
    };
